...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_entities):
    ...
    if not all(k in config_data for k in [CONF_API_TOKEN, CONF_FROM_NUMBER, CONF_COUNTRY_CODE]):
        _LOGGER.debug("Missing required configuration data for WhatsPie notify platform.")
        return
...
async def async_register_notify_service(hass: HomeAssistant, entity: NotifyEntity):
    ...
    _LOGGER.debug("WhatsPie notify service '%s' registered successfully.", notify_service_name)
...
async def async_send_whatsapp_text_message(hass: HomeAssistant, to, message, api_token, from_number) -> bool:
...
class WhatsPieNotificationService(NotifyEntity):
    ...
    async def async_send_message(self, service_call: ServiceCall) -> None:
        ...
        _LOGGER.debug("async_send_message called with message: %s, target: %s", message, target)
