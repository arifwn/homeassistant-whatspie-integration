    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                from_number = user_input[CONF_FROM_NUMBER].lstrip("0")  # Remove leading "0" if present
    ...
    async def _test_credentials(self, api_token, full_number):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise ValueError(f"API validation error: {response.status_code} - {response.text}")
    ...
    async def async_step_init(self, user_input=None):
        ...
        options_schema = vol.Schema(
            {   
                vol.Required(
                    CONF_API_TOKEN, 
                    default=self.config_entry.options.get(CONF_API_TOKEN, self.config_entry.data.get(CONF_API_TOKEN))
                ): cv.string,
                vol.Required(
                    CONF_FROM_NUMBER, default=self.config_entry.options.get(CONF_FROM_NUMBER, "")
                ): cv.string,
                vol.Required(
                    CONF_COUNTRY_CODE, default=self.config_entry.options.get(CONF_COUNTRY_CODE, "")
                ): cv.string,
            }
        )
